...
import { readVulnrichmentCache, readGithubAdvisoryCache, readLatestDataset } from './store.mjs';
import {
 ...
 * function. The chain reads the three envelopes back
 * from the Blob stores and uses the precomputed
 * per-Blob public hashes to compute the composite
 * publicStateHash.
 *
...
export async function runDatasetPublicationChain({
  ...
  if (!githubAdvisoryCache || !githubAdvisoryCache.githubAdvisoryPublicHash || typeof githubAdvisoryCache.githubAdvisoryPublicHash !== 'string') {
    return { skipped: true, reason: 'github-advisory-public-hash-missing' };
  }

  // 3. Read the current OSV projection reference.
  const osvProjection = await readCurrentOsvProjection(store);

  // 4. Invoke the V6.1 dataset-bound publication.
  let result = null;
  try {
    result = await publishDatasetBound(store, {
      datasetEnvelope,
      vulnrichmentCache,
      githubAdvisoryCache,
      osvProjection,
  ...
  if (!result) {
    return { skipped: true, reason: 'no-result' };
  }
  if (result.skipped) {
  ...
  return { published: true, publicIntelligenceVersion: result.publicIntelligenceVersion, ...result };
}
